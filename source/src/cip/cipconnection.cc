...
CipError CipConn::openConsumingPointToPointConnection( Cpf* aCpf, ConnMgrStatus* aExtError )
{
    ...
    // CRITICAL FIX: Use kEIP_IoUdpPort (44818) for binding
    // Most clients expect to send I/O data to this port
    SockAddr peers_destination(     // a.k.a "me"
                kEIP_IoUdpPort,     // Standard EtherNet/IP I/O port (44818)
                DEFAULT_BIND_IPADDR
                );
    ...
    if( kCIPsterFanucScanner )
    {
        aCpf->AddTx( kSockAddr_O_T, peers_destination );
    }

    // CRITICAL FIX: Use kEIP_IoUdpPort for remote source port
    SockAddr remotes_source(                // a.k.a "peer"
                kEIP_IoUdpPort,             // Use I/O port instead of custom port
...
CipError CipConn::openProducingPointToPointConnection( Cpf* aCpf, ConnMgrStatus* aExtError )
{
    ...
    // Create the destination address explicitly
    ...
    // CRITICAL FIX: Add SAII for O->T communication
    aCpf->AddTx( kSockAddr_O_T, destination );
