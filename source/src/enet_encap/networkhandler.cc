...
int UdpSocketMgr::createSocket( const SockAddr& aSockAddr )
{
   ...
   const int one = 1;  // Move variable declaration to the top
   ...
   if( udp_sock == kSocketInvalid )
   {
    ...
    // For Linux, SO_REUSEADDR helps with port reuse, especially for standard ports like 44818
    // Keep this option since it protects against "Address already in use" errors
